"""Ejecutar desde la carpeta de entrega: python3 -m unittest discover -s tests -v."""
...
def translation(tx, ty):
    """Construye una matriz de traslación bidimensional."""
    return np.array([[1, 0, tx], [0, 1, ty], [0, 0, 1]], dtype=np.float64)


class PanoramaBoundsTests(unittest.TestCase):
    """Verifica límites, traslaciones y validaciones del canvas."""

    def test_identity_has_no_extra_row_or_column(self):
        """Comprueba que la identidad conserva el tamaño exacto."""
    ...
    def test_translations_with_repository_image(self):
        """Comprueba dos traslaciones sobre una imagen real."""
        path = Path(__file__).parents[1] / "imgs" / "cuadro_1.jpg"
    ...
    def test_fractional_limits_round_outward(self):
        """Comprueba el redondeo hacia afuera de límites fraccionales."""
    ...
    def test_projective_division(self):
        """Comprueba la división por la coordenada homogénea."""
    ...
    def test_invalid_inputs(self):
        """Rechaza imágenes vacías y homografías inválidas."""
        img = np.zeros((20, 40), dtype=np.uint8)
